...
-- 1. BANK_T에 연락처(BANK_TEL) 칼럼을 추가하시오
  ALTER TABLE BANK_T ADD BANK_TEL VARCHAR(15 BYTE);
...
-- 5. CUSTOMER_T에 등급(GRADE) 칼럼을 추가하시오. 'VIP', 'GOLD', 'SILVER', 'BRONZE' 중 하나의 값을 가지도록 설정하시오.
  ALTER TABLE CUSTOMER_T ADD GRADE VARCHAR2(6 BYTE) CHECK(GRADE= 'VIP' OR GRADE= 'GOLD'OR GRADE='SILVER' OR GRADE='BRONZE');
                                                  --CHECK(GRADE IN('VIP','GOLD','SILVER','BROZNE'));
...
-- 7. CUSTOMER_T의 테이블명을 CUST_T로 수정하시오
  ALTER TABLE CUSTMOER_T RENAME TO CUST_T;
...
/*
   ...
   1. PK 
    ...
    2. FK
      1) 추가       : ALTER TABLE 자식 테이블명 ADD CONSTRAINT 제약조건명 FOREIGN KEY(칼럼명) REFERENCES 부모테이블명(참조할칼럼명) [옵션]
      2) 삭제       : ALTER TABLE 테이블명      DROP CONSTRAINT 제약조건명
      3) 비활성화   : ALTER TABLE               DISABLE CONSTRAINT 제약조건명
      4) 활성화     : ALTER TABLE               ENABLE CONSTRAINT 제약조건명
   
*/

--FK_CUSTOMER_BANK 제약조건을 비활성화하시오.
ALTER TABLE CUST_T DISABLE CONSTRAINT FK_CUSTMOER_BANK;


--FK CUSTOMERBANK 제약조건을 활성화 하시오
 ALTER TABLE CUST_T ENABLE CONSTRAINT FK_CUSTMOER_BANK;