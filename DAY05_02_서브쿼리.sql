...
SELECT *
  FROM EMPLOYEE_T
 WHERE SALARY > (SELECT AVG(SALARY)  --함수를 결과 반환해서 단일행
                   FROM EMPLOYEE_T);
...
SELECT A.*
  FROM (SELECT ROW_NUMBER() OVER(ORDER BY SALARY DESC) AS RN, EMPLOYEE_ID, SALARY, HIRE_DATE,LAST_NAME
          FROM EMPLOYEES) A
...
-- 1. (비상관)부서번호가 50인 부서에 근무하는 사원의 사원번호, 사원명, 부서명 조회히시오.
SELECT EMPLOYEE_ID
     , LAST_NAME
...
-- 2. (상관)부서번호가 50인 부서에 근무하는 사원의 사원번호, 사원명, 부서명 조회히시오.
...
CREATE TABLE 과장명단 (EMP_NO,NAME,POSITION, GENDER, HIRE_DATE, SALARY)
AS(SELECT EMP_NO,NAME,POSITION, GENDER, HIRE_DATE, SALARY 
  FROM EMPLOYEE_T
 WHERE 1 = 2);
INSERT INTO 과장명단 (EMP_NO,NAME,POSITION, GENDER, HIRE_DATE, SALARY)
