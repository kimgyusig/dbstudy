...
/*
    ...
    2. 어느 한 테이블에 존재하는 데이터는 조회되지 않는다.
    3. ANSI 문법
        SELECT 조회할 칼럼, ...
...
SELECT E.EMP_NO, E.NAME, E.DEPART, D.DEPT_NAME
  FROM DEPARTMENT_T D INNER JOIN EMPLOYEE_T E          -- PK 가있는 테이블을 왼쪽에 조인에서 왼쪽이 검색할 칼럼이다.
    ON D.DEPT_NO = E.DEPART;           --조인 때 별명 사용하기     PK 가 왼쪽에 조인에서 왼쪽이 검색할 칼럼이다. 드라이브 테이블라고 부른다.
...
SELECT E.EMP_NO, E.NAME, D.DEPT_NAME
  FROM DEPARTMENT_T D RIGHT JOIN EMPLOYEE_T E    --조회할 사람이 오른쪽에있다.
    ON D.DEPT_NO = E.DEPART;
    
-- 2. 부서별 사원수 조회하시오. 부서명, 사원수 사원이 없으면 0으로 조회하시오
...
SELECT E.EMPLOYEE_ID, E.LAST_NAME, D.DEPARTMENT_ID, D.DEPARTMENT_NAME, L.LOCATION_ID, L.STREET_ADDRESS
 FROM L.LOCATIONS L INNER JOIN D.DEPARTMENT D
   ON L.LOCATION_ID = D.LOCATION_ID INNER JOIN EMPLOYEES E
   ON D.DEPARTMENT_ID = E.DEPARTMENT_ID;                  --PK가 왼쪽에 있어 성능이 더 좋다.
...
-- 관계
-- 나는 여러 사원과 관계를 맺는다.
-- 나(EMPLOYEES ME)                너님들(EMPLOYESS YOU)
