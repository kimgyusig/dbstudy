...
/*
   트랜잭션
   1. 한 번에 수행해야 하는 작업을 의미한다. (원자성, All or Noting)
...
-- 부서 테이블에 행 삽입하기
 ...
 --사원테이블에 행 삽입하기
 INSERT INTO EMPLOYEE_T VALUES(EMPLOYEE_SEQ.NEXTVAL, '구창민', 1, '과장', 'M', '95/05/01', 5000000); --날짜는 하이픈(-) 또는 슬래시(/)
 ...
 */
 ...
 -- 1. DEPRATMENT_T에서 부서번호(DEPT_NO)가 3인 부서의 지역(LOCATUIN)을 '인천'으로 수정하시오
 UPDATE DEPARTMENT_T
   SET LOCATION = '인천'  -- SET절의 등호(=)는 대입 연산자
 WHERE DEPT_NO =  3;      -- WHERE절의 등호(=)는 동등비교 연산자
 
 -- 2. EMPLOYEE_T 부서번호(DEPART)가 2인 부서의 사원들의 기본급(SALARY)을 10% 인상하시오
UPDATE EMPLOYEE_T
 ...
 */
 ...
 -- 2. EMPLOYEE_T에서 부서번호(DEPART)RK 1과 4인 사원을 삭제하시오.
 DELETE 
   FROM EMPLOYEE_T
 WHERE DEPART = 1 OR DEPART = 4;     --WHERE PEPART IN(1,4)
 -- 3. DEPARTMENT_T에서 부서번호(DEPT_NO) 2인 부서를 삭제하시오. (부서번호가 2인 사원은 있다. 부서가 없어지면 사원 정보가 참조 무결성에 위배된다. 참조무결성 위배에 대비해서 ON DELETE SET NULL 옵션을 주었으므로 사원 테이블의 부서번호가 NULL로 함께 수정된다.)
DELETE 
  FROM DEPARTMENT_T 
 WHERE DEPT_NO = 2;
